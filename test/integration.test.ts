...
// ─── Reasoning-mode wiring (AC-5) ────────────────────────────────────────
//
// End-to-end coverage of the per-turn reasoning-mode pass: the wiring
// (index.ts) reads `cfg.reasoningMode` from the already-resolved
// config and forwards it to `applyThreeTierTrim` as a `TrimOptions`
// field. The pass runs at the top of every `context` event before
// the three-tier trim accounts the trimmable mass, so the chosen
// mode reduces the trimmable total before the three-tier trim sees
// it (the operator's intent — reasoning tokens aren't reused later
// for context anyway).
//
// These tests use the production `defaultSummaSummarizer` (the
// Python `summa` subprocess) — the policy-level mock is in
// `policy.test.ts`. The integration test isolates the wiring-layer
// path: `cfg.reasoningMode` reaches `applyThreeTierTrim` per the
// branch's contract (`feat/reasoning-wiring-23`, commit 7152b52).
//
// The reasoning-mode env is set per-test via `CONFIG_ENV.reasoningMode`
// (env > file > default precedence). The file channel is pointed at
// a non-existent path in the module-level `before` so the env is the
// only input that matters.

describe("context handler — reasoning-mode wiring (AC-5)", () => {
	let sReasoning: string | undefined;
	beforeEach(() => {
		sReasoning = process.env[CONFIG_ENV.reasoningMode];
		delete process.env[CONFIG_ENV.reasoningMode];
	});
	afterEach(() => {
		if (sReasoning === undefined) delete process.env[CONFIG_ENV.reasoningMode];
		else process.env[CONFIG_ENV.reasoningMode] = sReasoning;
	});

	// ── Mode 'keep' ───────────────────────────────────────────

	it("mode 'keep' (env=1): thinking blocks flow through unchanged", async () => {
		process.env[CONFIG_ENV.reasoningMode] = "1";
		const pi = await loadExtension();
		const event = {
			messages: [
				userMsg("dispatch"),
				// Three assistant turns, each carrying a thinking
				// block. The latest is preserved in all modes; the
				// older two are touched by the mode (summa or drop
				// — not 'keep').
				{
					role: "assistant",
					content: [
						{ type: "thinking", thinking: "earliest thinking block to preserve" },
				...
				},
				{
					role: "assistant",
					content: [
						{ type: "thinking", thinking: "middle thinking block to preserve" },
		...
		const result = (await invokeContext(pi, event)) as { messages: Array<Record<string, unknown>> };
		// All three thinking blocks survive verbatim.
		const flat = result.messages.map((m) => typeof m.content === "string" ? m.content : JSON.stringify(m.content)).join("\n");
		assert.ok(flat.includes("earliest thinking block to preserve"), "oldest turn's thinking block must survive in mode 'keep'");
		assert.ok(flat.includes("middle thinking block to preserve"), "middle turn's thinking block must survive in mode 'keep'");
		assert.ok(flat.includes("latest thinking block to preserve"), "latest turn's thinking block must survive in mode 'keep'");
		// No summa envelope (mode 'keep' does not invoke the
		// summarizer in the reasoning-mode pass).
		assert.ok(!flat.includes("[summa:"), "no summa envelope in mode 'keep'");
	});

	// ── Mode 'summa-all-but-latest' ───────────────────────────

	it("mode 'summa-all-but-latest' (env=2, the default): the latest turn's thinking block survives, the pass runs over the older turns", async () => {
		// The wiring-layer contract: `cfg.reasoningMode` reaches
		// `applyThreeTierTrim`, the reasoning-mode pass runs, and
		// the latest turn's thinking block survives verbatim. The
		// actual rewrite of the older turns' thinking text is
		// exercised in the policy unit test (which uses a
		// deterministic mock summarizer); the production summa
		// subprocess can fall back to the source text when the
		// fallback path runs, so this integration test asserts the
		// wiring seam shape, not the substring replacement.
		process.env[CONFIG_ENV.reasoningMode] = "2";
		const pi = await loadExtension();
		const longThinking = "The model is reasoning through the problem step by step, considering the trade-offs and the operator's intent, weighing each option carefully before making a decision. ";
		const event = {
			messages: [
				userMsg("dispatch"),
				{
					role: "assistant",
					content: [
						{ type: "thinking", thinking: longThinking + "earliest unique marker" },
						{ type: "text", text: "answer 1" },
					],
				},
				{
					role: "assistant",
					content: [
						{ type: "thinking", thinking: longThinking + "middle unique marker" },
						{ type: "text", text: "answer 2" },
					],
				},
				{
					role: "assistant",
					content: [
						{ type: "thinking", thinking: "latest thinking to preserve verbatim" },
						{ type: "text", text: "answer 3" },
					],
				},
			],
		...
		const flat = result.messages.map((m) => typeof m.content === "string" ? m.content : JSON.stringify(m.content)).join("\n");
		// The latest turn's thinking block survives verbatim (the
		// cross-mode invariant). This is the load-bearing
		// wiring-layer assertion: the latest must be preserved
		// regardless of the production summa subprocess's output.
		assert.ok(
			flat.includes("latest thinking to preserve verbatim"),
			"latest turn's thinking block must survive verbatim in mode 'summa-all-but-latest'",
		);
		// The non-thinking text blocks survive verbatim (the
		// reasoning-mode pass only touches type: 'thinking' blocks).
		assert.ok(flat.includes("answer 1"));
		assert.ok(flat.includes("answer 2"));
		assert.ok(flat.includes("answer 3"));
	});

	// ── Mode 'drop-all-but-latest' ────────────────────────────

	it("mode 'drop-all-but-latest' (env=3): thinking blocks of older turns are removed, latest is preserved, non-thinking blocks in same message are preserved", async () => {
		process.env[CONFIG_ENV.reasoningMode] = "3";
		const pi = await loadExtension();
		const event = {
			messages: [
				userMsg("dispatch"),
				// Older turn: thinking + text + toolCall. The
				// thinking is dropped; text and toolCall survive.
				{
					role: "assistant",
					content: [
						{ type: "text", text: "OLD TEXT" },
						{ type: "thinking", thinking: "old thinking to drop" },
						{ type: "toolCall", name: "kept-tool", arguments: { a: 1 } },
					],
				},
				// Latest turn: thinking + text. Both survive.
				{
					role: "assistant",
					content: [
						{ type: "thinking", thinking: "latest thinking to preserve verbatim" },
						{ type: "text", text: "latest text to preserve verbatim" },
					],
				},
			],
		};
		...
		assert.ok(
			flat.includes("latest thinking to preserve verbatim"),
			"latest turn's thinking block must survive verbatim in mode 'drop-all-but-latest'",
		);
		// The older turn's thinking block is gone.
		assert.ok(
			!flat.includes("old thinking to drop"),
			"old turn's thinking block must be removed in mode 'drop-all-but-latest'",
		);
		// Non-thinking blocks in the older turn survive.
		assert.ok(flat.includes("OLD TEXT"), "non-thinking text block in the same message must survive");
		assert.ok(flat.includes("kept-tool"), "non-thinking toolCall block in the same message must survive");
		// The latest turn's text survives verbatim.
		assert.ok(flat.includes("latest text to preserve verbatim"));
	});

	// ── Wiring seam: the resolved value reaches applyThreeTierTrim ──
	//
	// The wiring-layer contract (feat/reasoning-wiring-23): the
	// resolved `cfg.reasoningMode` is passed in the `TrimOptions`
	// bag to `applyThreeTierTrim`. The three modes produce
	// distinguishable results on a fixture with multiple
	// thinking-bearing assistant turns; a non-thinking fixture
	// produces the same result under all three modes (the pass is
	// a no-op when no thinking blocks exist). The two assertions
	// below cover the seam end-to-end.

	it("wiring seam: a session with no thinking blocks produces the same output under all three modes", async () => {
		// Build the same session three times under three modes.
		// No thinking blocks are present, so the reasoning-mode
		// pass is a no-op and the three outputs are byte-equal.
		function buildEvent() {
			return {
				messages: [
					userMsg("dispatch"),
		...
		// Mode 'keep' (env=1).
		process.env[CONFIG_ENV.reasoningMode] = "1";
		const piKeep = await loadExtension();
		const resultKeep = (await invokeContext(piKeep, buildEvent())) as { messages: Array<Record<string, unknown>> };
		// Mode 'summa-all-but-latest' (env=2).
		process.env[CONFIG_ENV.reasoningMode] = "2";
		const piSumma = await loadExtension();
		const resultSumma = (await invokeContext(piSumma, buildEvent())) as { messages: Array<Record<string, unknown>> };
		// Mode 'drop-all-but-latest' (env=3).
		process.env[CONFIG_ENV.reasoningMode] = "3";
		const piDrop = await loadExtension();
		const resultDrop = (await invokeContext(piDrop, buildEvent())) as { messages: Array<Record<string, unknown>> };
		// Serialize the output of each so the comparison is
		// order-independent on object identity.
		const keep = resultKeep.messages.map((m) => JSON.stringify(m));
		const summa = resultSumma.messages.map((m) => JSON.stringify(m));
		const drop = resultDrop.messages.map((m) => JSON.stringify(m));
		assert.deepEqual(keep, summa, "no thinking blocks → mode 'keep' and 'summa-all-but-latest' produce identical output");
		assert.deepEqual(keep, drop, "no thinking blocks → mode 'keep' and 'drop-all-but-latest' produce identical output");
	});

	it("wiring seam: env unset falls through to the default 'summa-all-but-latest' (no env, no file override)", async () => {
		// No env override. The file channel is pointed at a
		// non-existent path (module-level `before`), so neither
		// channel sets `reasoningMode`. The resolver returns the
		// default `summa-all-but-latest`. The wiring passes that
		// default into `applyThreeTierTrim` and the pass runs.
		// We assert the wiring seam (the default reaches the
		// policy and the latest turn's thinking block survives),
		// not the production summa subprocess's output shape.
		const pi = await loadExtension();
		const longThinking = "The model is reasoning through the problem step by step, considering the trade-offs and the operator's intent. ";
		const event = {
			messages: [
				userMsg("dispatch"),
				{
		...
		const result = (await invokeContext(pi, event)) as { messages: Array<Record<string, unknown>> };
		const flat = result.messages.map((m) => typeof m.content === "string" ? m.content : JSON.stringify(m.content)).join("\n");
		// Default 'summa-all-but-latest' preserves the latest turn's
		// thinking block verbatim — the cross-mode invariant. The
		// default is reached end-to-end (config → wiring → policy).
		assert.ok(
			flat.includes("latest thinking to preserve verbatim"),
			"default 'summa-all-but-latest' must preserve the latest turn's thinking block",
		);
		// The non-thinking text blocks survive verbatim.
		assert.ok(flat.includes("answer 1"));
		assert.ok(flat.includes("answer 2"));
		assert.ok(flat.includes("answer 3"));
	});
