...
`usage.totalTokens` does not trigger a reset. Providers can report stale totals, cache-inclusive totals, or status percentages that do not describe the current stream. The local visible-content estimate, after protected mass and system-prompt accounting, is the sole reset predicate.
